...
public class VitalSignServiceImpl implements VitalSignService{
    ...
    @Override
    public Flux<VitalSign> findByFilter(String from,String to,String regNo) {
        Query query = new Query(Criteria
                .where("createdDate").gte(CVUtil.toISODate(from)).lte(CVUtil.toISODate(to))
                .and("regNo").is(regNo)
        );
        log.info("query : "+query);
        return template.find(query, VitalSign.class);
    }

    @Override
    public Mono<VitalSign> findByRegNo(String regNo) {
